<div id="content" class="site-content" tabindex="-1">
	<div class="container">
		...
		<div id="sidebar" class="sidebar" role="complementary" style="margin-top: <?php if (isset($__category)) echo '50px;';
																					else echo '520px;'; ?>">
...
<script>
	...
	function getGadgets(page) {
		$('span.page-numbers').text(page)
		let link = location.pathname
		let split = link.split('/')
		console.log(split)

		let search = location.search + '&page=' + page

		let loclink = '/fetch/fetch_all_product/' + search
		// position = split[split.length - 1]
		if (split.length > 2) {
			loclink = '/fetch/' + split[split.length - 1] + '/' + search
			console.log(split[split.length - 1] == '')
			if (split[split.length - 1] == '') loclink = '/fetch/' + split[split.length - 2] + '/' + search
		}
		...
		fetch(loclink)
			...
			.then(data => {
				...
				let searchName = search.get('sort')

				if (searchName == 'price-asc') {
				 gadgets.sort((a, b) => a.product_price - b.product_price)
				} else if (searchName == 'price-desc') {
				 gadgets.sort((a, b) => b.product_price - a.product_price)
				} else if (searchName == 'rating') {
				 gadgets.sort((a, b) => b.rating.rating - a.rating.rating)
				} else if (searchName == 'popular') {
					gadgets.reverse()
				}
	...
	function gadgetsItem(e) {
		...
		if(obj){
			...
			if (objectLength > 0) {
				...
				let item = obj[keys[keys.length-1]];
						...
						<div class="media-body media-middle">
							<div class="row">
								...
								<div class="col-xs-12">
									...
									<a class="button product_type_simple add_to_cart_button ajax_add_to_cart" data-product_sku="" data-product_id="" data-quantity="1" href="/${category_name}/${e.product_slug}" rel="nofollow"><i class="heart-fill"></i></a>
